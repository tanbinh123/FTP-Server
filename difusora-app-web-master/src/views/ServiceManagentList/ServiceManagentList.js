...
const ServiceManagentList = () => {
  ...
  useEffect(() => {
    ...
    if (mounted)
      fetchServices(page, size, order, orderBy);
  ...
  const fetchServices = (page, size, order, orderBy) => {
    ...
    axios.get('/v1/service', { params }).then(response => {
  ...
  const handleRequestSort = async (event, property) => {
    ...
    fetchServices(page, size, (isAsc ? 'desc' : 'asc'), property);
  };
...
export default ServiceManagentList;
