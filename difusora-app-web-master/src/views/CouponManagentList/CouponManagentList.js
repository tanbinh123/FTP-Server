...
const CouponManagentList = () => {
  ...
  useEffect(() => {
    ...
    if (mounted) 
      fetchCoupon(page, size, order, orderBy);
  ...
  const fetchCoupon = (page, size, order, orderBy) => {
    ...
    axios.get('/marketplace-coupon-service/v1/coupon', { params }).then(response => {
  ...
  const handleRequestSort = async (event, property) => {
    ...
    fetchCoupon(page, size, (isAsc ? 'desc' : 'asc'), property);
  };
...
export default CouponManagentList;
